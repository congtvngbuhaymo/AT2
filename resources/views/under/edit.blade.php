                    ...
                    <div class="row justify-content-center">
                        <div class="col-md-8">
                            <div class="card shadow-lg p-6 lg:p-8">
                                <form method="POST" action="{{ route('under.update', $under->id) }}">
                                    @csrf
                                    @method('PUT')

                                    <div class="mb-3">
                                        <label for="name" class="form-label text-sm fw-medium text-gray-900 dark:text-white">Name</label>
                                        <input type="text" name="name" id="name" value="{{ old('name', $under->name) }}" class="form-control form-control-lg rounded-lg px-3 py-2" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="batch" class="form-label text-sm fw-medium text-gray-900 dark:text-white">Batch no.</label>
                                        <input type="number" name="batch" id="batch" value="{{ old('batch', $under->batch) }}" class="form-control form-control-lg rounded-lg px-3 py-2" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="email" class="form-label text-sm fw-medium text-gray-900 dark:text-white">Email</label>
                                        <input type="email" name="email" id="email" value="{{ old('email', $under->email) }}" class="form-control form-control-lg rounded-lg px-3 py-2" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="address" class="form-label text-sm fw-medium text-gray-900 dark:text-white">Address</label>
                                        <input type="text" name="address" id="address" value="{{ old('address', $under->address) }}" class="form-control form-control-lg rounded-lg px-3 py-2" required>
                                    </div>

                                    <div class="mb-3 d-flex justify-content-end">
                                        <a href="{{ route('under.index') }}" class="btn btn-secondary btn-lg rounded-lg px-4 py-2 me-2">Cancel</a>
